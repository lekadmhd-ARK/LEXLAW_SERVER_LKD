...
{
 public function upload(Request $request){$v=$request->validate(['pdf'=>'required|file|mimes:pdf|max:20480','regulation_id'=>'required|exists:regulations,id']);$path=$request->file('pdf')->store('regulations','r2');return response()->json(['path'=>$path,'download_url'=>Storage::disk('r2')->temporaryUrl($path,now()->addMinutes(15))]);}
