...
curl -skL -b /tmp/cookies -c /tmp/cookies -d "_token=$TOKEN&email=superadmin@lawlex.test&password=Admin123!" https://lexlaw.arktech.id/login -o /dev/null
...
curl -skL -b /tmp/cookies -c /tmp/cookies -X POST https://lexlaw.arktech.id/ai/contract-review \
  ...
  | jq . 2>/dev/null || curl -skL -b /tmp/cookies -c /tmp/cookies -X POST https://lexlaw.arktech.id/ai/contract-review -H "Accept: application/json" -d "_token=${TOKEN2}" -d "contract_text=test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test test"
