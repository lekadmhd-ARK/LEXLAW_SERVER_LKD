<x-layouts.base title="Tambah Istilah">
    <div>
        <div class="cr-hero"><div class="page-head">
            <div>
                <div class="eyebrow">📚 Glosari Hukum</div>
                <h1 class="page-title">Tambah Istilah</h1>
                <p class="page-desc">Tambahkan istilah hukum baru ke kamus.</p>
            </div>
            <a href="/legal-glossary" class="btn btn-primary">← Kembali</a>
        </div></div>

        <div style="max-width:720px">
            <form method="POST" action="/legal-glossary" style="display:flex;flex-direction:column;gap:16px">
                @csrf
                <div>
                    <label style="font-size:13px;font-weight:600;color:var(--muted)">Istilah (term) *</label>
                    <input type="text" name="term" required value="{{ old('term') }}" style="width:100%;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--bg);color:var(--text);margin-top:6px">
                </div>
                <div>
                    <label style="font-size:13px;font-weight:600;color:var(--muted)">Singkatan / Alias</label>
                    <input type="text" name="singkatan" value="{{ old('singkatan') }}" style="width:100%;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--bg);color:var(--text);margin-top:6px">
                </div>
                <div>
                    ...
                    <select name="kategori_hukum" style="width:100%;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--bg);color:var(--text);margin-top:6px">
                        ...
                        <option value="{{ $c }}" {{ old('kategori_hukum') === $c ? 'selected' : '' }}>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label style="font-size:13px;font-weight:600;color:var(--muted)">Definisi Singkat</label>
                    <input type="text" name="definisi_singkat" value="{{ old('definisi_singkat') }}" style="width:100%;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--bg);color:var(--text);margin-top:6px">
                </div>
                <div>
                    <label style="font-size:13px;font-weight:600;color:var(--muted)">Penjelasan Lengkap *</label>
                    <textarea name="penjelasan_lengkap" required rows="5" style="width:100%;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--bg);color:var(--text);margin-top:6px;line-height:1.6">{{ old('penjelasan_lengkap') }}</textarea>
                </div>
                <div>
                    <label style="font-size:13px;font-weight:600;color:var(--muted)">Contoh Penerapan / Kasus</label>
                    <textarea name="contoh_implementasi" rows="3" style="width:100%;padding:12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--bg);color:var(--text);margin-top:6px;line-height:1.6">{{ old('contoh_implementasi') }}</textarea>
                </div>
                <button type="submit" class="btn btn-primary" style="padding:12px 20px;font-size:15px">Simpan Istilah</button>
            </form>
        </div>
    </div>
